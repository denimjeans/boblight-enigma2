...
int boblight_getlightnr(void* vpboblight, int x, int y)
{
  CBoblight* boblight = reinterpret_cast<CBoblight*>(vpboblight);
  return boblight->GetLightNr(x,y);
}
...
void boblight_addbitmap(void* vpboblight, unsigned char* bmp, int xsize, int ysize)
{
  CBoblight* boblight = reinterpret_cast<CBoblight*>(vpboblight);
  boblight->AddBitmap(bmp, xsize, ysize);
}
