...
//this function put all data in buffer, The grabber can call this function each time when fame is created.
int CBoblight::SendToBuffer()
{
  timeshift=5;
  //Insert whole frame into buffer
  ...
  for (int i = 0; i < m_lights.size(); i++)
  {
    ...
    for (int r=0; r < 3; r++)
    {
    	lights[i][r][pointer] = (unsigned char *)ToString(rgb[r]).c_str(); //Float to string
    	//lights[i][r][pointer] = (unsigned char *)&rgb[r]; //Float to char
    }
  }
  counter_shift=pointer-timeshift; //Calculate new timeshift
  pointer++; //Cycles between 0 and 255... so we can save 255 frames
...
int CBoblight::SendRGB(int sync, int* outputused)
{
  ...
  for (int i = 0; i < m_lights.size(); i++)
  {
    ...
    //Fill with data from Char
    rgb0 << lights[i][0][counter_shift];
    rgb1 << lights[i][1][counter_shift];
    rgb2 << lights[i][2][counter_shift];
    
    istringstream(rgb0.str()) >> rgb[0];
    istringstream(rgb1.str()) >> rgb[1];
    istringstream(rgb2.str()) >> rgb[2];
    
    rgb_data = ToString(rgb[0]) + " " + ToString(rgb[1]) + " " + ToString(rgb[2]);
    cout << "RGBData from buffer : " << rgb_data << endl;
    
    data += "set light " + m_lights[i].m_name + " rgb " + rgb_data +"\n";
