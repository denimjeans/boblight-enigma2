...
void CGrabber::ResetPixelData(){
	...
	//Set blankcounter to 0;
	blank_count=0;
}
	
unsigned long** CGrabber::create_2D_char_array( int numrows, int numcols){
	
	unsigned long **data;
	data = (unsigned long **) malloc(numrows * sizeof(unsigned long *));
	if(data == NULL){
		free(data);
	...
	/* Allocate integer memory for the second dimension ; */
	register int i;
	for(i = 0; i < numrows; i++) {
		data[i] = (unsigned long *) malloc(numcols * sizeof(unsigned long));
		if(NULL == data[i]){
		free(data[i]);
...
bool CGrabber::Setup()
{
	...
	if (!detectSTB()) { //If unknown then return.
		m_error = "detect STB failed!\n";
		return 1; //Stop boblight
	}
	printf("[grabber] found STB-Type %d\n",stb_type);
  
	// Prepare memory access
	if (stb_type == BRCM7335 || stb_type == BRCM7401 || stb_type == BRCM7400 || stb_type == BRCM7405 || stb_type == BRCM7358 || stb_type == BRCM7356 || stb_type == BRCM7325)
	{
	...
	// make sure to get enough memory
	...
	memset(video,0,m_size * m_size * 2 * 3);

	// Set some vars to default values
	sb_toggle = false;
	v_=v__=save_total=save_bottom=save_top=cyclecount=cycle_save=m_fps=delay=0;
  
	// for debug output...
	filename_count = 0;	
	printf("[grabber] Maximum intermediate-picture-size is %dpx\n",m_size);
	
	if (m_debug) 
		printf("[grabber] Debug mode enabled\n");
	if (m_sync) 
		printf("[grabber] Sync mode enabled\n");
	if (m_blackbar) 
		printf("[grabber] Blackbar mode enabled\n");

	printf("[grabber] Found %d lights\n",nrlights);
...
bool CGrabber::detectSTB()
{
	...
	if (pipe)
	{
		while (fgets(buf,sizeof(buf),pipe))
		{
			...
			else if (strcasestr(buf,"7405(with 3D)")) //VU Ultimo with 3D
			{
				stb_type = BRCM7405;
				break;
			}
			...
			{
				stb_type = BRCM7335; //Use same as 7335
				break;
			}
...
{	
	...
	{   
		...
		//set to global
		if(yres > 0 && xres > 0){ 
			yres_old=abs(yres); xres_old=adjust_x;
			printf("[grabber] Setting Resolution to %dx%d (Source %dx%d)\n",xres_old,yres_old,xres_orig,yres_orig);
			boblight_setscanrange(m_boblight, adjust_x, yres);
		}
	}
	...
	//Check if blackbar detection is enabled and there is no blank, then create new sb_end
	if(m_blackbar && save_total > 0 && sb_toggle && !blank)
	{
		end = (yres-save_top);
	}
...
int CGrabber::file_scanf_lines(const char *filename, const char *fmt, ...)
{
	char *line = NULL;
	size_t n = 0;
	int ret = 0;
...
bool CGrabber::grabVideo()
{
	...
	{
		if(m_debug){
			printf("[Debug] Mainmemory data: <Memmapping failed>\n");
		}		
		return false;
	}
	...
	if (xres != stride) {
		...
		if(m_debug){
			printf("[Debug] X-Resolution != stride: %d\n",stride);
		}
		blank = true;return false;
	}
	...
	if (stb_type == BRCM7401 || stb_type == BRCM7358 || stb_type == BRCM7405 )
	{
		...
		if (memory_tmp == MAP_FAILED || memory_tmp == NULL) {
			if(m_debug){
				printf("[Debug] Mainmemory: <Memmapping failed>\n");
			}
			blank = true;return false;
		}
	...
	{
		timeshift=0; //Timeshift not needed for dma
		...
		{
			if(m_debug){
			   printf("[Debug] Got invalid stride value from the decoder: %d\n", stride);
			}
			blank = true;return false;
		}
	...
	xres	= stride;
	xres_orig=xres; yres_orig=yres;	// store original resolution

	//
	// decode luma & chroma plane or lets say sort it
	//
	unsigned int x,y;
	unsigned int luna_mem_pos = 0, chroma_mem_pos = 0;
	...
	//deceide on how much data we want to skip.., default we skip 2 lines.
	skiplines=2;
	while (yres/skiplines > m_size) {
		skiplines=skiplines*2;
	}
	...
	if (yres%2 == 1)
		yres++;		// drop one line to make the number even again
	
	yres = yres/skiplines;	   
	
	//blocksize for reading
	int blocksize=chr_luma_stride;

	//fill luma and chroma with data
	for (x=0; x < stride; x+=chr_luma_stride)
	{
		...
		{
			memcpy(luma + dat1, memory_tmp + luna_mem_pos, blocksize); // luma
			luna_mem_pos += chr_luma_stride*skiplines;

			if (y<ofs2)
			{
				memcpy(chroma + dat1, memory_tmp + offset + chroma_mem_pos, blocksize); // chroma
				chroma_mem_pos += chr_luma_stride*skiplines;
			}	
...
bool CGrabber::SaveBMP() {
	...
	for (y=0; y < yres; y+=2)
	{
		for (x=0; x < xres; x+=skiplines*4)
		{
			...
			U=chroma[pos_chr+3];	//0
			V=chroma[pos_chr+2];	//1
			...
			GU=yuv2rgbtable_gu[U]; // BV<->RU swapped (aiograb used RGB, we want BGR for bmp
			GV=yuv2rgbtable_gv[V];
			BV=yuv2rgbtable_bv[V];
			RU2=yuv2rgbtable_ru[U2]; 
			GU2=yuv2rgbtable_gu[U2];
			GV2=yuv2rgbtable_gv[V2];
			BV2=yuv2rgbtable_bv[V2];
					
			// now we do 4*2 pixels on each iteration this is more code but much faster 
			Y=yuv2rgbtable_y[luma[pos_luma+3]]; 
			video[pos_rgb]=CLAMP((Y + RU)>>16);
			video[pos_rgb+1]=CLAMP((Y - GV - GU)>>16);
			video[pos_rgb+2]=CLAMP((Y + BV)>>16);

			Y=yuv2rgbtable_y[luma[pos_luma+2]];
			video[pos_rgb+3]=CLAMP((Y + RU)>>16);
			video[pos_rgb+4]=CLAMP((Y - GV - GU)>>16);
			video[pos_rgb+5]=CLAMP((Y + BV)>>16);

			Y=yuv2rgbtable_y[luma[pos_luma+1]]; 
			video[pos_rgb+6]=CLAMP((Y + RU2)>>16);
			video[pos_rgb+7]=CLAMP((Y - GV2 - GU2)>>16);
			video[pos_rgb+8]=CLAMP((Y + BV2)>>16);
	
			Y=yuv2rgbtable_y[luma[pos_luma]];
			video[pos_rgb+9]=CLAMP((Y + RU2)>>16);
			video[pos_rgb+10]=CLAMP((Y - GV2 - GU2)>>16);
			video[pos_rgb+11]=CLAMP((Y + BV2)>>16);
			...
			pos_rgb  +=12;	// skip forward for the next group of 4 pixels
		}
	...
	sprintf (buffer, "/tmp/boblight_enigma2_%04d.bmp", filename_count++);
	...
	printf("[grabber] Saving image to %s (%d,%d)\n",buffer,xres,yres);
	// write data
	for (y=yres-1; y>=0 ; y-=1) {
		fwrite(video+(y*xres*output_bytes),xres*output_bytes,1,fd2);
	}
	fclose(fd2); //close fd2
}

void CGrabber::CalcBeamTop()
{
   	int x,y;
	...
	for (y=0; y < yres/2; y+=2)
	{
		...
		{   
			//16 - 128 - 128 is black
			if ((luma[x + y * xres] > 22) || (chroma[(x&~1) + (y>>1)*xres] > 133) || (chroma[(x&~1) + (y>>1)*xres + 1] > 133)){						  
				y_=1; //Ok, it seems that there is color;
			}else{
				x_++; //Count lines with black color 
			}			
			//printf("X:%d Y:%d Luma:%d ChromaU:%d ChromaV:%d\n",x, y , luma[x + (y * xres)],chroma[(x&~1) + (y>>1)*xres],chroma[(x&~1) + (y>>1)*xres + 1]);
		 }
	}
	...
	y_=0;y=0;x_=0; //set all vars to 0
	
	//check bottom
	for (y=(yres/2); y < yres; y+=2)
	{
		for (x=0; x < xres; x+=skiplines*4)
		{   
			//16 - 128 - 128 is black
	...
	//Calculate new resolution without blacklines
	int bottom   = abs(((yres_orig-(bottom_lines))-yres_orig)/skiplines)+1;
	int top	  = abs(((yres_orig-(top_lines))-yres_orig)/skiplines)+1;
	
	if (bottom%2 == 1) bottom++; //make bottom even if itś not
	if (top%2	== 1) top++;	//make top even if itś not
	
	int total	= abs(bottom+top);   
	
	//set new yres without blacklines
	yres_new = yres-(total);

	//check if sbb is diferent at save_total, then set sb and set new scanrange
	if(total != save_total && abs(total-save_total) >= 4 && sb_toggle)
	{
		printf ("\n[grabber][blackbar] Black bars changed. totalOld:%d totalNew:%d\n",save_total,total);
		...
		printf ("[grabber][blackbar] Setting Resolution to %dx%d \n",xres_orig/skiplines,yres_new);
		
		//Set new scanrange
		boblight_setscanrange(m_boblight, xres_orig/skiplines, yres_new);
	}
	
	//If the lines are 2 or higher and v_ and v__are higher then 10.
	if ((v_>=10 && v__ >=10) && !sb_toggle && total > 2) 
	{
		save_total  =  total; //save
		save_bottom =  top;
		save_top	=  bottom;
		...
		printf ("\n[grabber][blackbar] Black bars detected. lines:%d\n",total);
		printf ("[grabber][blackbar] Setting Resolution to %dx%d \n",xres_orig/skiplines,yres_new);
	...
	//if the lines are below 6 and sb_toggle is true or ....
	if ((v_ < 2 && v__ < 2) && sb_toggle) 
	{
		save_total  =  total; //save
		save_bottom =  0;
		save_top	=  0;
		...
		printf ("\n[grabber][blackbar] Black bars no longer detected. lines:%d\n",total);
		printf ("[grabber][blackbar] Setting Resolution to %dx%d\n",xres,yres);

		//Set new scanrange
		boblight_setscanrange(m_boblight, xres_orig/skiplines, yres);
	}
