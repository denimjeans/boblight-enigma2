...
int Run()
{
  while(!stop)
  {
    ...
    grabber->SetPicdump(g_flagmanager.m_picdump);		// Set picdump enabled or disbaled.
    grabber->SetBlackbar(g_flagmanager.m_blackbar);		// Set blackbardetection enabled or disabled/
    grabber->SetTimeshift(g_flagmanager.m_timeshift);   // For testing..
