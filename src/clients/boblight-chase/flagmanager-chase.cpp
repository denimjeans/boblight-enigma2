...
void CFlagManagerChase::PostGetopt(int optind, int argc, char** argv)
{
  //check if a color was given
  if (optind == argc)
    throw string("no mode given");

  if (!StrToInt(argv[optind], m_mode))
   m_mode = 0;
...
void CFlagManagerChase::PrintHelpMessage()
{
  cout << "Usage: boblight-Chase [OPTION] color\n";
  cout << "\n";
